...
function helpmenu() {
  ...
  printf "Syntax: 4c-multipe-sample-plots.sh [options] <template sample XLSX file> <multiple sample plot template XSLX file> <genomic coordinates> <output dir>\n"
  printf "\n"
...
TEMP=`getopt -o hi:s: -l inputdir:,group-only,ylim-low:,ylim-high:,add-enhancers:,add-promoters:,shading:,ci-alpha:,add-vertical-line: -n '4cmultiplots' -- "$@"`
eval set -- "$TEMP"
