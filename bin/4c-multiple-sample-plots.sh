...
function helpmenu() {
  ...
  printf " %s\t%s\n" "--add-vertical-line=[POS1,POS2,...]" "plot black vertical line at positions POS1[,POS2,...], must be comma-delimited"
  printf " %s\t%s\n" "--cairopng" "plot images also in PNG using Cairo (PDFs are generated by default in R)") | column -t -s $'\t'
...
perl -I$LIBDIR $SCRIPTDIR/make-multiple-sample-plots.pl $SAMPLETABLE $MULTITABLE $ORGANISMDATABASE $SCRIPTDIR $COORDINATES $SHADING $INPUTDIR $OUTPUTDIR $YLIMLOW $YLIMHIGH $ENHANCERFILE $PROMOTERFILE $VERTLINES $CIALPHA $CAIROPNG
