...
TEMP=`getopt -o hi:s: -l inputdir:,group-only,ylim-low:,ylim-high:,add-enhancers:,add-promoters:,shading:,add-vertical-line:,ci-alpha:,cariopng -n '4cplots' -- "$@"`
eval set -- "$TEMP"
...
perl -I$LIBDIR $SCRIPTDIR/make-plots.pl $SAMPLETABLE $ORGANISMDATABASE $SCRIPTDIR $COORDINATES $SHADING $INPUTDIR $OUTPUTDIR $GROUPONLY $YLIMLOW $YLIMHIGH $ENHANCERFILE $PROMOTERFILE $VERTLINES $CIALPHA $CAIROPNG $@
