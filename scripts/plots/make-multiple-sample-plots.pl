...
die "Arguments: <template file> <multiple sample template file> <organism database> <basedir> <genomic coordinates> <shading> <input dir> <output dir> <ylim low> <ylim high> <enhancer file> <promoter file>" unless scalar(@ARGV)>=11;

my ($sampletable,$multisampletable,$organismdatabase,$basedir,$genomecoord,$shading,$inputdir,$outputdir,$ylimlow,$ylimhigh,$enhancerfile,$promoterfile) = @ARGV;
...
for( my $i = 0; $i < $nr; $i++ ) {
  ...
  my $output = `Rscript $basedir/plot-4c-signal.r $genomecoord $shading $statsfile $ylimlow $ylimhigh $enhancerfile $promoterfile $pdfoutput $pngoutput $fileargs 2>&1`;

  die "Failed to generate output for $jointkey (arguments $genomecoord $shading $statsfile $ylimlow $ylimhigh $enhancerfile $promoterfile $pdfoutput $pngoutput $fileargs), messages: $output" unless $? == 0;
}
